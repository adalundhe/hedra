...
class Connection:
    ...
    async def connect(self):
        ...
        await self.connection.add_stream(stream_config)
    ...
    async def execute(self, query) -> None:
        ...
        if statement.type == 'update':
            ...
            response = await self.connection.update(event)

        elif statement.type == 'stat_query':
            query = statement.to_query()
            response = await self.connection.get_field_stat(query)

        elif statement.type == 'field_query':
            query = statement.to_query()
            response = await self.connection.get_field_stats(query)
        
        else:
            query = statement.to_query()
            response = await self.connection.get_stream_stats(query)
