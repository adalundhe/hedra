...
class Graph:
    ...
    def __init__(
        ...
        self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Changed status to - {GraphStatus.INITIALIZING.name} - from - {GraphStatus.IDLE.name}')
        self.logger.filesystem.sync['hedra.core'].info(f'{self._graph_metadata_log_string} - Changed status to - {GraphStatus.INITIALIZING.name} - {GraphStatus.IDLE.name}')

        self.transitions_graph = []
        self._transitions: List[List[Transition]] = []
        self._results = None

        self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Found - {len(stages)} - stages')
        self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Found - {len(stages)} - stages')
        ...
        for stage in stages:
            ...
            self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Adding dependencies for stage - {stage.__name__}')
            self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Adding dependencies for stage - {stage.__name__}')

            for dependency in stage.dependencies:
                if self.graph.nodes.get(dependency.__name__):
                    
                    self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Adding edge from stage - {dependency.__name__} - to stage - {stage.__name__}')
                    self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Adding edge from stage - {dependency.__name__} - to stage - {stage.__name__}.')
    ...
    def assemble(self):
        ...
        self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Changed status to - {GraphStatus.ASSEMBLING.name} - from - {GraphStatus.INITIALIZING.name}')
        self.logger.filesystem.sync['hedra.core'].info(f'{self._graph_metadata_log_string} - Changed status to - {GraphStatus.ASSEMBLING.name} - from - {GraphStatus.INITIALIZING.name}')

        # If we havent specified a Validate stage for save aggregated results,
        # append one.
        if len(self.instances.get(StageTypes.VALIDATE)) < 1:
            self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Prepending {StageTypes.VALIDATE.name} stage')
            self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Prepending {StageTypes.VALIDATE.name} stage')
        ...
        self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Prepending {StageTypes.IDLE.name} stage')
        self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Prepending {StageTypes.IDLE.name} stage')
        ...
        if len(self.instances.get(StageTypes.ANALYZE)) < 1:
            self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Appending {StageTypes.ANALYZE.name} stage')
            self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Appending {StageTypes.ANALYZE.name} stage')
        ...
        if len(self.instances.get(StageTypes.SUBMIT)) < 1:
            self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Appending {StageTypes.SUBMIT.name} stage')
            self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Appending {StageTypes.SUBMIT.name} stage')
        ...
        self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Appending {StageTypes.COMPLETE.name} stage')
        self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Appending {StageTypes.COMPLETE.name} stage')

        self._append_stage(StageTypes.COMPLETE)

        self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Generating graph stages and transitions')
        self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Generating graph stages and transitions')
        ...
        self.logger.hedra.sync.debug(f'{self._graph_metadata_log_string} - Assembly complete')
        self.logger.filesystem.sync['hedra.core'].debug(f'{self._graph_metadata_log_string} - Assembly complete')

    async def run(self):

        execution_start = time.monotonic()

        await self.logger.hedra.aio.debug(f'{self._graph_metadata_log_string} - Changed status to - {GraphStatus.RUNNING.name} - from - {GraphStatus.ASSEMBLING.name}')
        await self.logger.filesystem.aio['hedra.core'].info(f'{self._graph_metadata_log_string} - Changed status to - {GraphStatus.RUNNING.name} - from - {GraphStatus.ASSEMBLING.name}')
        ...
        for transition_group in self._transitions:
            ...
            async with self.logger.spinner as status_spinner:
                ...
                for transition in transition_group:
                    ...
                    await self.logger.filesystem.aio['hedra.core'].info(f'{self._graph_metadata_log_string} - Executing stage Transition - {transition.transition_id} - from stage - {transition.from_stage.name} - to stage - {transition.to_stage.name}')
                    await self.logger.filesystem.aio['hedra.core'].info(f'{self._graph_metadata_log_string} - Executing stage - {transition.from_stage.name}:{transition.from_stage.stage_id}')
                ...
                for transition in transition_group:
                    await status_spinner.system.debug(f'{self._graph_metadata_log_string} - Completed stage Transtition - {transition.transition_id} -  from stage - {transition.from_stage.name} - to stage - {transition.to_stage.name}')
                ...
                await self.logger.filesystem.aio['hedra.core'].debug(f'{self._graph_metadata_log_string} - Completed -  {completed_transitions_count} - transitions')
                ...
                for error, next_stage in results:
                    ...
                    if next_stage == StageTypes.ERROR:
                        ...
                        await self.logger.filesystem.aio['hedra.core'].info(f'{self._graph_metadata_log_string} - Changed status to - {GraphStatus.FAILED.name} - from - {GraphStatus.RUNNING.name}')

                        await status_spinner.system.error(f'{self._graph_metadata_log_string} - Encountered error executing stage - {error.from_stage}')
                        await self.logger.filesystem.aio['hedra.core'].error(f'{self._graph_metadata_log_string} - Encountered error executing stage - {error.from_stage}')
        ...
        if self.status == GraphStatus.RUNNING:
            ...
            await self.logger.filesystem.aio['hedra.core'].info(f'{self._graph_metadata_log_string} - Changed status to - {GraphStatus.COMPLETE.name} - from - {GraphStatus.RUNNING.name}')
