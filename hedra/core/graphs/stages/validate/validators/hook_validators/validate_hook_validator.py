...
class ValidateHookValidator(BaseHookVaidator):
    ...
    async def validate(self, hook: ValidateHook):
        ...
        try:
            ...
            assert call.__code__.co_argcount > 1, f"Missing required argument 'hook' for @validate hook {hook.name}:{hook.hook_id}"
            assert call.__code__.co_argcount < 3, f"Too many args. - @validate hook {hook.name}:{hook.hook_id} only requires 'events_group' as additional arg."
            ...
            for name in hook.names:
                ...
                await call(hook_for_validation.call)
