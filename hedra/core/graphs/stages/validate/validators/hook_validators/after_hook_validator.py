...
class AfterHookValidator(BaseHookVaidator):
    ...
    async def validate(self, hook: Union[AfterHook, BaseEvent]):
        ...
        try:
            ...
            assert call is not None, f"Method is not not found on stage or was not supplied to @after hook - {hook.name}:{hook.hook_id}"
            assert call.__code__.co_argcount > 2, f"Missing required arguments 'action' or 'result' for @after hook {hook.name}:{hook.hook_id}"
            assert call.__code__.co_argcount < 4, f"Too many args. - @after hook {hook.name}:{hook.hook_id} only requires 'action' and 'result' as additional args."
