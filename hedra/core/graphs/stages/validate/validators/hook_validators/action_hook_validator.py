...
class ActionHookValidator(BaseHookVaidator):
    ...
    async def validate(self, hook: Union[ActionHook, BaseEvent]):
        ...
        try:
            ...
            assert call is not None, f"Method is not not found on stage or was not supplied to @action hook - {hook.name}:{hook.hook_id}"
            assert call.__code__.co_argcount == 1, f"Too many args. - @action hook {hook.name}:{hook.hook_id} requires no additional args."
