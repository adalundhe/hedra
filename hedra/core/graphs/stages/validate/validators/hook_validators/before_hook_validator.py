...
class BeforeHookVaidator(BaseHookVaidator):
    ...
    async def validate(self, hook: Union[BeforeHook, BaseEvent]):
        ...
        try:
            ...
            assert call.__code__.co_argcount > 2, f"Missing required argument 'action' for @before hook {hook.name}:{hook.hook_id}"
            assert call.__code__.co_argcount < 4, f"Too many args. - @before hook {hook.name}:{hook.hook_id} only requires 'action' as additional arg."
